...
function hex_to_ascii(strHex)
 {
	var hex  = strHex.toString();
	var str = '';
...
function iseData(hexStr) {
  var asciiStr = hex_to_ascii(hexStr);
  var str = asciiStr.split(":");
  var data = {};
  
  data.Temperature = parseFloat(str[0]);
  data.Humidity = parseFloat(str[1]);
  data.Pressure = parseFloat(str[2]);
  data.Lux = parseFloat(str[3]);
  //data = "data: {Temperature: " + parseFloat(str[0]) + ", Humidity: " + parseFloat(str[1]) + ", Pressure: " + str[2] + ", Lux: " + str[3] + "}";
